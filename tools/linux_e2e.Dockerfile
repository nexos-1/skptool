...
# Linux-Pruefumgebung fuer tools/linux_e2e.sh: Python 3.12 und Blender 5.2.0, ohne echten Bildschirm
# (die Fenster-Tests laufen mit xvfb-run auf einem virtuellen).
...
# Systembibliotheken fuer Blender (wie im CI-Job), xz zum Entpacken. Fuer die Fenster-Tests des
# Live-Modus ein virtueller Bildschirm: xvfb (mit xvfb-run) und xauth; OpenGL kommt als
# Software-Rendering aus Mesa (llvmpipe in libgl1-mesa-dri).
RUN apt-get update -q \
 && apt-get install -y -q --no-install-recommends \
      libegl1 libgl1 libgl1-mesa-dri libegl-mesa0 libxi6 libxkbcommon0 libxxf86vm1 libxfixes3 \
      libxrender1 libsm6 xz-utils xvfb xauth \
...
USER pruefer
# Fenster-Tests standardmaessig aus (ganze Suite ohne Bildschirm); linux_e2e.sh schaltet sie fuer den
# Schritt mit xvfb-run wieder ein
